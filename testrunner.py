...
def skipIfFailed(test_case: Type[unittest.TestCase] = None,
                 test_name: Union[str, FunctionType] = None, tag: str = None):
	"""
	...
	if test_case is not None and test_name is not None and not hasattr(
			test_case, test_name):
...
def import_module(path: str) -> Tuple[
	Optional[ModuleType], Optional[Tuple[Type, Exception, TracebackType]]]:
	"""
...
def _test_wrapper(test_func):
	...
	def wrapper(self):
		...
		if isinstance(error, SystemExit):
			raise RuntimeError(
				"You should not be using exit, quit or sys.exit").with_traceback(
				error.__traceback__)
...
class AttributeGuesser:
	...
	def get_wrapped_object(cls, attr_guesser):
		if not isinstance(attr_guesser, AttributeGuesser):
			raise ValueError(
				'attr_guesser must be an instance of AttributeGuesser')
		return object.__getattribute__(attr_guesser,
	...
	def __getattribute__(self, key: str):
		...
		return getattr(
			object.__getattribute__(self, '_AttributeGuesser__object'), key)
...
class TestCase(unittest.TestCase, metaclass=_TestCaseMeta):
	...
	def assertMultiLineEqual(self, first: str, second: str,
	                         msg: Optional[str] = None, strip: bool = False):
		"""
	...
	def assertFunctionDefined(self, obj: Union[ModuleType, Type],
	                          function_name: str, params: int):
		...
		if not inspect.isfunction(func):
			if inspect.ismethoddescriptor(func):
				self.fail(
					msg=f"'{obj_name}.{function_name}' needs to be implemented")
	...
	def assertIsSubclass(self, sub_class: Type, parent_class: Type):
		if not issubclass(sub_class, parent_class):
			self.fail(
				msg=f"'{sub_class.__qualname__}' should be a subclass of '{parent_class.__qualname__}'")
	...
	def assertDocString(self, obj: Union[Type, Callable], name: str = None):
		...
		if doc is None or doc == '':
			self.fail(
				msg=f"Documentation string is required for '{obj.__qualname__}'")

	def assertRecursiveDocString(self, cls: Type,
	                             name: str = None, module: ModuleType = None):
		root_cls = cls
		method = getattr(cls, name)

		if method is None:
			self.fail(
				msg=f"Class {cls.__name__} doesn't have method '{name}'")
		while True:
			doc = getattr(method, "__doc__")
			if doc is not None and doc != "":
				# Found docstrings
				return
			# Assume just 1 parent class
			cls = cls.__base__
			if getattr(module, cls.__name__, None) is None:
				self.fail(f"Method '{name}' from class "
						  f"'{root_cls.__name__}' does not have docstrings in "
						  f"any of its superclasses in {module.__name__}")
			method = getattr(cls, name, None)
			if method is None:
				self.fail(f"Method '{name}' from class "
						  f"'{root_cls.__name__}' does not have "
						  f"docstrings in any of its superclasses.")
...
class TestResult(unittest.TestResult):
	...
	def __init__(self, stream=None, descriptions=None, verbosity=None):
		...
		self.results: Dict[
			str, Dict[str, Tuple[TestCase, TestOutcome]]] = OrderedDict()
	...
	def _apply_skip(self, test: TestCase,
	                test_item: Union[Type[TestCase], FunctionType]):
		"""
	...
	def check_outcome(self, test, test_cls_name, test_name, tag):
		...
		if test_name is not None:
			...
			for err, tag_ in test_case.aggregated_tests:
				...
				outcome != TestOutcome.PASS for _, outcome in
				test_results.values()):
			return False
...
class TestResultWithMessage(TestResult):
	def __init__(self, stream=None, descriptions=None, verbosity=None):
		super().__init__(stream, descriptions, verbosity)
		self.results: Dict[str, Dict[str, Tuple[TestCase, TestOutcome, str]]] = \
			OrderedDict()

	def add_outcome(self, test: TestCase, outcome: TestOutcome,
	                reason: str = ""):
		self.results[test.__class__.__name__][test.name] = (
			test, outcome, reason)
	...
	def addSkip(self, test: TestCase, reason: str):
		super().addSkip(test, reason)
		self.add_outcome(test, TestOutcome.SKIP, reason)
	...
	def check_outcome(self, test, test_cls_name, test_name, tag):
		...
		if test_name is not None:
			test_case, outcome, reason = test_results[test_name]
			if tag is None:
				return outcome == TestOutcome.PASS
			...
			for err, tag_ in test_case.aggregated_tests:
				...
				outcome != TestOutcome.PASS for _, outcome, *_ in
				test_results.values()):
			return False
	...
	def to_dict(self):
		return {
			test_cls:
				{name: "PASSED" if outcome == TestOutcome.PASS else
				f"FAILED: {reason}" for name, (test, outcome, reason) in
				 res.items()}
			for test_cls, res in self.results.items()
		}
...
class TestMaster:
		...
		for name, path in scripts:
			...
			if error:
				self._import_errors.append(
					(path, self.format_error(path, error)))
	...
	def print_results(self, failed_tests: List[Tuple[str, TestCase, str]],
	                  result: TestResult):
		# print summary
		...
		for test_cls, test_cases in result.results.items():
			passes = sum(outcome == TestOutcome.PASS
			             for _, outcome, *_ in test_cases.values())
			print(f'{test_cls}: {passes}/{len(test_cases)}')
			for _test_name, (test, outcome, *_) in test_cases.items():
		...
		if failed_tests:
			...
			for flavour, test, msg in failed_tests:
				if self._args.show_tb_duplicates:
					...
					self.print_error(flavour, test,
					                 DUPLICATE_MSG if msg == prev else msg.strip())
					prev = msg
	...
	def output_results(self, all_tests: List[TestCase], result: TestResult):
		...
		fails, skips = len(result.failures) + len(result.errors), len(
			result.skipped)
	...
	def run(self, test_cases: List[Union[TestCase, Type[TestCase]]]) -> \
			Optional[TestResult]:
		...
		with RedirectStdIO(stdin=True,
			...
			runner = unittest.TextTestRunner(stream=None,
			                                 verbosity=0,
			                                 resultclass=TestResultWithMessage)
